...
export default function Home() {
  ...
  const abrirWhatsApp = async () => {
    const url =
      'https://wa.me/5515996893760?text=Olá!%20Quero%20saber%20mais%20sobre%20o%20EcoVoucher.';
  ...
  useEffect(() => {
    ...
    const carregarDados = async () => {
      try {
        ...
        if (uRes.ok) {
          // pontos, quando disponíveis no payload
          setPontos(uRes.data?.pontos ?? 0);

          if (usuario.tipo === 'pf') {
            // pegada: prioriza serviço dedicado
            try {
              const ultima = await PegadaService.obterUltimaPontuacao(documento);
              setPegada(ultima?.pontuacao ?? (uRes.data?.pontuacao ?? null));
            } catch {
              setPegada(uRes.data?.pontuacao ?? null);
            }

            setIcones([
              {
                imagem: require('../../assets/imagensEco/historicoIcon.png'),
                rota: '/historicopontos',
                label: 'Histórico \nde Pontos',
              },
              {
                imagem: require('../../assets/imagensEco/catalogoIcon.png'),
                rota: '/catalogovoucherspf',
                label: 'Vouchers \npara Troca',
              },
              {
                imagem: require('../../assets/imagensEco/pontoColetaIcon.png'),
                rota: '/pontoscoleta',
                label: 'Pontos \nde Coleta',
              },
            ]);
        ...
        if (usuario.tipo === 'pj') {
          ...
          setVouchersUtilizados(estRes.data?.totalComprados ?? 0);

          setIcones([
            {
              imagem: require('../../assets/imagensEco/gerarVoucherIcon.png'),
              rota: '/catalogorecompensapj',
              label: 'Gerar Voucher',
            },
            {
              imagem: require('../../assets/imagensEco/validarVoucherIcon.png'),
              rota: '/validarvoucherpj',
